...
public class OVRGrabber : MonoBehaviour
{
    ...
    [SerializeField]
    protected Transform m_handTransform = null;
    [SerializeField]
    protected int speed;
    [SerializeField]
    protected int curve;
    //[SerializeField]
    //protected bool throw_trigger;
    public static bool ballcatch = false;

    public static int Triggercheck = 0;
    public static bool Catch_Error = false;
    /**/
    ...
    void OnUpdatedAnchors()
    {
        ...
        /**/
        //PrimaryHandTriggerが中指でPrimaryIndexTriggerが人差し指の入力を取っている。
        switch (Triggercheck) {
        ...
        //Debug.Log(OVRInput.Get(OVRInput.Axis1D.PrimaryHandTrigger, m_controller));
        //Debug.Log(OVRInput.Get(OVRInput.Axis1D.PrimaryIndexTrigger, m_controller));
        /**/
    ...
    protected void GrabBegin()
    {
        ...
        foreach (OVRGrabbable grabbable in m_grabCandidates.Keys)
        {
            ...
            for (int j = 0; j < grabbable.grabPoints.Length; ++j)
            {
                ...
                /**/
                if (grabbableCollider == null)//掴めないエラーが出たときに呼ばれる。（消えたボールのコライダーを読み込もうとしている時）
                {
#if UNITY_EDITOR
                    Debug.Log("Catch_Error_grabbableCollider");
#endif
                   
                    m_grabCandidates.Remove(grabbable);//ボールが掴める判定内でDestroyした時に、判定外に出た情報を渡すと、コライダーの情報がリセットされる。
    ...
    protected void MoveGrabbedObject(Vector3 pos, Quaternion rot, bool forceTeleport = false)
    {
        ...
        Debug.Log("catch");
#endif
        Vector3 Distance = hand.position - (pos + rot * m_grabbedObjectPosOff);//判定内でトリガーを押した時のボールの座標から手の中心座標を引いている。
        /**/

        Rigidbody grabbedRigidbody = m_grabbedObj.grabbedRigidbody;
        Vector3 grabbablePosition = (pos + rot * m_grabbedObjectPosOff) + Distance;//上の式を足して手の中心にボールを動かし続ける。
