...
namespace Prince_rapidity_99
{
    ...
    public class PlayComponent: Microsoft.Xna.Framework.DrawableGameComponent
    {
        ...
        Vector3[] glassRoadPosition = new Vector3[51];

        Vector3[] glassClifPosition = new Vector3[21];

        Vector3[] dartClifPosition = new Vector3[97];

        Vector3[] stonePosition = new Vector3[296];

        Vector3[] stoneRoadPosition = new Vector3[10];

        Vector3[] needlePosition = new Vector3[42];
        ...
        GlassRoad[] glassRoad = new GlassRoad[51];
        GlassClif[] glassClif = new GlassClif[21];
        DartClif[] dartClif = new DartClif[97];
        Stone[] stone = new Stone[296];
        StoneRoad[] stoneRoad = new StoneRoad[10];
        Needle[] needle = new Needle[42];
        Dart[] dart = new Dart[312];
        Stage stage;

        bool ended = false;
        bool[] glassRoadFlg = new bool[51];
        bool[] DartClifFlg = new bool[97];
        bool[] glassClifFlg = new bool[21];
        bool[] stoneFlg = new bool[296];
        bool[] stoneRoadFlg = new bool[10];
        bool[] needleFlg = new bool[42];
        ...
        public override void Initialize()
        {
            ...
            {
                glassRoadPosition[glassRoadCount] = new Vector3(1340.0f + (20.0f * i), -390.0f, 0.0f);
                glassRoadCount++;
            }

            glassRoadPosition[glassRoadCount] = new Vector3(1360.0f, -290.0f, 0.0f);
            glassRoadCount++; //38

            for (int i = 0; i < 7; i++) //45
            {
                glassRoadPosition[glassRoadCount] = new Vector3(1380.0f, -250.0f, 0.0f);
                glassRoadCount++;
            }

            for (int i = 0; i < 5; i++) //50
            {
                glassRoadPosition[glassRoadCount] = new Vector3(1600.0f, -290.0f, 0.0f);
                glassRoadCount++;
            }

            glassRoadPosition[glassRoadCount] = new Vector3(1360.0f, -310.0f, 0.0f);
            glassRoadCount++; //51
            ...
            glassClifPosition[15] = new Vector3(1440.0f, -370.0f, 0.0f);
            glassClifPosition[16] = new Vector3(1460.0f, -350.0f, 0.0f);
            glassClifPosition[17] = new Vector3(1480.0f, -330.0f, 0.0f);
            glassClifPosition[18] = new Vector3(1440.0f, -310.0f, 0.0f);
            glassClifPosition[19] = new Vector3(1380.0f, -290.0f, 0.0f);
            glassClifPosition[20] = new Vector3(1340.0f, -270.0f, 0.0f);
            ...
            for (int i = 0; i < 11; i++) ////83
            {
                dartClifPosition[dartClifCount] = new Vector3(1320.0f, -10.0f + (-20.0f * i), 0.0f);
                dartClifCount++;
            }
            dartClifPosition[dartClifCount] = new Vector3(1340.0f, -290.0f, 0.0f);
            dartClifPosition[dartClifCount] = new Vector3(1340.0f, -310.0f, 0.0f);
            dartClifCount++;
            dartClifCount++; //85

            for (int i = 0; i < 3; i++) //88
            {
                dartClifPosition[dartClifCount] = new Vector3(1480.0f, -350.0f + (-20.0f * i), 0.0f);
                dartClifCount++;
            }

            for (int i = 0; i < 5; i++) //93
            {
                dartClifPosition[dartClifCount] = new Vector3(1580.0f, -310.0f + (-20.0f * i), 0.0f);
                dartClifCount++;
            }
            ...
            for (int i = 0; i < 2; i++) ////248
            {
                for (int y = 0; y < 3; y++)
                {
                    stonePosition[stoneCount] = new Vector3(1220.0f + (20.0f * y), -170.0f + (-20.0f * i), 0.0f);
                    stoneCount++;
                }
            }
            for (int i = 0; i < 4; i++) //264
            {
                for (int y = 0; y < 4; y++)
                {
                    stonePosition[stoneCount] = new Vector3(1240.0f + (20.0f * y), -210.0f + (-20.0f * i), 0.0f);
                    stoneCount++;
                }
            }
            for (int i = 0; i < 3; i++)
            {
                stonePosition[stoneCount] = new Vector3(1280.0f, -350.0f, 0.0f);
                stoneCount++;
            }

            stonePosition[stoneCount] = new Vector3(1580.0f, -290.0f, 0.0f);
            stoneCount++;
            ...
            stoneRoadPosition[stoneRoadCount] = new Vector3(1280.0f, -330.0f, 0.0f);
            stoneRoadCount++; ////10
            ...
            for (int i = 0; i < 3; i++) //33
            {
                needlePosition[needleCount] = new Vector3(1220.0f + (20.0f * i), -330.0f, 0.0f);
                needleCount++;
            }
            for (int i = 0; i < 2; i++) //35
            {
                needlePosition[needleCount] = new Vector3(1330.0f + (20.0f * i), -390.0f, 0.0f);
                needleCount++;
            }
            for (int i = 0; i < 2; i++) //37
            {
                needlePosition[needleCount] = new Vector3(1400.0f + (20.0f * i), -310.0f, 0.0f);
                needleCount++;
            }
            for (int i = 0; i < 4; i++) //41
            {
                needlePosition[needleCount] = new Vector3(1500.0f + (20.0f * i), -390.0f, 0.0f);
                needleCount++;
            }

            needlePosition[needleCount] = new Vector3(1580.0f, -270.0f, 0.0f);
            needleCount++; //42
            ...
            for (int count = 27; count < needleCount; count++)
            {
                needle[count] = new Needle(camera);
            }
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            //stoneの描画
            for (int count = 0; count < 242; count++)
            {
